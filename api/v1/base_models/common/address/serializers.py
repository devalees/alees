...
class AddressSerializer(serializers.ModelSerializer):
    ...
    Note:
    -----
    This serializer is designed to be used as a nested serializer within parent entity serializers
    (e.g., OrganizationSerializer, UserProfileSerializer). Addresses are not managed through
    standalone endpoints but rather through their parent entities.

    The serializer includes comprehensive validation for all fields, including custom fields,
    to ensure data integrity when addresses are created or updated through parent entities.
    """
    ...
    def validate_custom_fields(self, value):
        ...
        for field_name, field_value in value.items():
            ...
            if field_name not in CUSTOM_FIELD_SCHEMA:
                allowed_fields = ", ".join(CUSTOM_FIELD_SCHEMA.keys())
            ...
            if (
                ...
                and len(field_value) > field_schema["max_length"]
            ):
                raise serializers.ValidationError(
                    f"'{field_name}' cannot exceed {field_schema['max_length']} "
                    "characters"
                )
        ...
        required_fields = [
            field_name
            for field_name, schema in CUSTOM_FIELD_SCHEMA.items()
