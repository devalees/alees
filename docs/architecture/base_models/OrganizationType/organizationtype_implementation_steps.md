...
## 3. Implementation Steps (TDD Workflow)
  ...
  [ ] Define the `OrganizationType` class in `api/v1/base_models/organization/models.py`.
  ...
  [ ] Define `OrganizationTypeFactory` in `api/v1/base_models/organization/tests/factories.py`:
      ...
      ```
  [ ] **(Test)** Write a simple test ensuring `OrganizationTypeFactory` creates valid instances. Test `django_get_or_create` works.

  ### 3.3 Admin Registration (`admin.py`)

  [ ] Create/Update `api/v1/base_models/organization/admin.py`.
  [ ] Define `OrganizationTypeAdmin`:
      ```python
  ...
  [ ] Create a new **Data Migration** file: `python manage.py makemigrations --empty --name populate_org_types api.v1.base_models.organization`.
  [ ] Edit the generated migration file (`..._populate_org_types.py`). Add `RunPython` operations to load essential types.
      ```python
  ...
  [ ] **Review generated migration file(s) carefully.** (Should create `OrganizationType` table, then the data migration).
  [ ] Run `python manage.py migrate` locally. Verify data loaded (e.g., via Admin).

  ### 3.6 Serializer Definition (`serializers.py`)

  [ ] **(Test First - Representation)** Write Integration Tests (`tests/integration/test_serializers.py`) verifying `OrganizationTypeSerializer` output format.
  [ ] Define `OrganizationTypeSerializer` in `api/v1/base_models/organization/serializers.py`:
      ```python
  ...
  [ ] **(Test First - Permissions/Basic Structure)** Write basic API Tests (`tests/api/test_endpoints.py`) for `/api/v1/organization-types/`. Test unauthenticated/authenticated access (likely read-only allowed).
  [ ] Define `OrganizationTypeViewSet` in `api/v1/base_models/organization/views.py`:
      ```python
      ...
      class OrganizationTypeViewSet(viewsets.ReadOnlyModelViewSet):
          ...
          """
          queryset = OrganizationType.objects.all() # Or filter active if needed
          serializer_class = OrganizationTypeSerializer
          permission_classes = [permissions.IsAuthenticatedOrReadOnly] # Allow read, restrict write
          # Use name (PK) for lookup by default if name is PK
          # lookup_field = 'name'
          filter_backends = [] # Add search/ordering if needed
          search_fields = ['name', 'description']
  ...
  [ ] Import `OrganizationTypeViewSet` in `api/v1/base_models/organization/urls.py`.
  [ ] Register the ViewSet with the router: `router.register(r'organization-types', views.OrganizationTypeViewSet)`.
  [ ] Ensure `organization.urls` is included in `api/v1/base_models/urls.py`.
  ...
  [ ] *(CRUD tests not applicable for ReadOnlyModelViewSet)*. Write tests for management API if implemented separately later.
...
[ ] Manually test via API client and verify data/management in Django Admin.
